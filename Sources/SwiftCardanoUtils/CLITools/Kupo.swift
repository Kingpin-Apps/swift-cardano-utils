    ...
    public mutating func start() throws {
        ...
        if let ogmiosConfig = configuration.ogmios {
            ...
            guard let socket = cardanoConfig.socket else {
                throw SwiftCardanoUtilsError.valueError("Cardano node socket path is required for kupo when Ogmios is not configured")
            }
            guard let config = cardanoConfig.config else {
                throw SwiftCardanoUtilsError.valueError("Cardano node config path is required for kupo when Ogmios is not configured")
            }
