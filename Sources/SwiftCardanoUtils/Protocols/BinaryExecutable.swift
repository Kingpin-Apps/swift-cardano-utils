...
extension BinaryExecutable {
        ...
        do {
            ...
            if process.terminationStatus == 0 {
                ...
                if let outputString = String(data: outputData, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines), !outputString.isEmpty {
                    ...
                    throw SwiftCardanoUtilsError.binaryNotFound(Self.binaryName)
                }
            } else {
                let errorData = errorPipe.fileHandleForReading.readDataToEndOfFile()
                let errorMessage = String(data: errorData, encoding: .utf8) ?? "Unknown error"
                throw SwiftCardanoUtilsError.commandFailed([Self.binaryName], "Failed to run command: \(errorMessage)")
            }
        } catch {
            throw SwiftCardanoUtilsError.commandFailed([Self.binaryName], "Failed to run command: \(error.localizedDescription)")
        }
    }
    ...
    public static func checkBinary(binary: FilePath) throws -> Void {
        ...
        {
            throw SwiftCardanoUtilsError.binaryNotFound("\(Self.binaryName) binary file not found: \(binary.string)")
        }
    ...
    public func checkVersion() async throws {        
        ...
        if currentVersion.compare(Self.mininumSupportedVersion, options: .numeric) == .orderedAscending {
            ...
            throw SwiftCardanoUtilsError.unsupportedVersion(currentVersion, Self.mininumSupportedVersion)
        }
    }
}
