    ...
    public struct VoteCommandImpl: CommandProtocol {
        ...
        public func registrationMetadata(
            ...
            guard votePublicKeys.count == voteWeights.count else {
                throw CardanoCLIToolsError.invalidParameters("Number of vote public keys (\(votePublicKeys.count)) must match number of vote weights (\(voteWeights.count))")
            }
            
            guard !votePublicKeys.isEmpty else {
                throw CardanoCLIToolsError.invalidParameters("At least one vote public key must be provided")
            }
