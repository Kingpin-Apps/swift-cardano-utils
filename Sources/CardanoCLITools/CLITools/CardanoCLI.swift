...
public struct CardanoCLI: BinaryInterfaceable {
    ...
    public func checkOnline() async throws {
        ...
        if syncProgress < 100.0 {
            throw CardanoCLIToolsError.nodeNotSynced(syncProgress)
        }
    }
    ...
    public func getSyncProgress() async throws -> Double {
        do {
            ...
            guard let syncProgressDouble = Double(chainTip.syncProgress) else {
                throw CardanoCLIToolsError.invalidOutput("Could not parse syncProgress as Double: \(chainTip.syncProgress)")
            }
    ...
    public func getEpoch() async throws -> Int {
        do {
            ...
            throw CardanoCLIToolsError.invalidOutput("Could not get epoch from node: \(error)")
        }
    }
    ...
    public func calculateEpochOffline() async throws -> UInt32 {
        ...
        guard let startDate = formatter.date(from: systemStartStr) else {
            throw CardanoCLIToolsError.invalidOutput("Could not parse systemStart date")
        }
    ...
    private func getScript(from referenceScript: [String: Any]) async throws -> ScriptType {
        guard let script = referenceScript["script"] as? [String: Any],
            ...
            throw CardanoCLIToolsError.valueError("Invalid reference script")
        }
        ...
        if scriptType == "PlutusScriptV1" {
            guard let cborHex = script["cborHex"] as? String,
                ...
                throw CardanoCLIToolsError.valueError("Invalid PlutusScriptV1 CBOR")
            }
            ...
            guard let cborHex = script["cborHex"] as? String,
                ...
                throw CardanoCLIToolsError.valueError("Invalid PlutusScriptV2 CBOR")
            }
            ...
            guard let jsonData = try? JSONSerialization.data(withJSONObject: script, options: [])
                ...
                throw CardanoCLIToolsError.valueError("Failed to serialize NativeScript JSON")
            }
    ...
    public func utxos(address: Address) async throws -> [UTxO] {
        ...
        guard let data = result.data(using: .utf8),
            ...
            throw CardanoCLIToolsError.valueError("Failed to parse UTxOs JSON")
        }
    ...
    public func signTransaction(txFile: FilePath, signingKeys: [FilePath]) async throws -> FilePath {
        do {
            ...
            throw CardanoCLIToolsError.commandFailed(["transaction", "sign"], "Unable to sign transaction: \(error)")
        }
    }
    ...
    public func submitTransaction(signedTxFile: FilePath, cleanup: Bool = false) async throws -> String {
        ...
        do {
            ...
            throw CardanoCLIToolsError.commandFailed(["transaction", "submit"], "Unable to submit transaction: \(error)")
        }
        ...
        do {
            ...
            throw CardanoCLIToolsError.commandFailed(["transaction", "txid"], "Unable to get transaction id: \(error)")
        }
