...
struct ConfigTests {
    ...
    @Test("Config initializes with all components")
    ...
    @Test("Config initializes with minimal components")
    ...
    @Test("Config initializes with only Ogmios")
    ...
    @Test("Config initializes with only Kupo")
    ...
    @Test("Config encodes to JSON correctly")
    ...
    @Test("Config decodes from JSON correctly")
    ...
    @Test("Config saves to file successfully")
    ...
    @Test("Config throws error when file already exists")
    func testSaveToExistingFile() throws {
        let config = Config(cardano: createTestCardanoConfig())
        let tempFile = createTempFile()
    ...
    @Test("Config loads from file successfully")
    func testLoadFromFile() async throws {
        ...
        // Load config
        let loadedConfig = try await Config.load(
    ...
    @Test("Config load throws error for non-existent file")
    func testLoadFromNonExistentFile() async {
        let nonExistentFile = FilePath("/tmp/non-existent-\(UUID().uuidString).json")
        
        await #expect(throws: Error.self) {
            _ = try await Config.load(path: nonExistentFile)
        }
    }
    
    @Test("Config initializes from ConfigReader with JSON data")
    ...
    @Test("Config default configuration contains required components")
    ...
    func testMalformedJSON() {
        ...
        #expect(throws: Error.self) {
            _ = try decoder.decode(Config.self, from: malformedJSON)
        }
    }
    ...
    func testRequiredFieldValidation() {
        ...
        #expect(throws: Error.self) {
            _ = try decoder.decode(Config.self, from: incompleteJSON)
        }
    }
}
