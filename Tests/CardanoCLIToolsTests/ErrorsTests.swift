...
struct CardanoCLIToolsErrorTests {
    ...
    func testBinaryNotFoundError() {
        ...
        let error = CardanoCLIToolsError.binaryNotFound(binaryPath)
    ...
    func testBinaryNotFoundErrorWithDifferentPaths() {
        ...
        for path in testPaths {
            let error = CardanoCLIToolsError.binaryNotFound(path)
    ...
    func testCommandFailedError() {
        ...
        let error = CardanoCLIToolsError.commandFailed(command, errorMessage)
    ...
    func testProcessAlreadyRunningError() {
        let error = CardanoCLIToolsError.processAlreadyRunning
    ...
    func testConfigurationMissingError() {
        ...
        let error = CardanoCLIToolsError.configurationMissing(config)
    ...
    func testConfigurationMissingErrorWithDifferentConfigurations() {
        ...
        let error = CardanoCLIToolsError.configurationMissing(configuration)
    ...
    func testDeviceErrorError() {
        ...
        let error = CardanoCLIToolsError.deviceError(deviceMessage)
    ...
    func testDeviceErrorWithVariousMessages() {
        ...
        for message in testMessages {
            let error = CardanoCLIToolsError.deviceError(message)
    ...
    func testInvalidOutputError() {
        ...
        let error = CardanoCLIToolsError.invalidOutput(outputMessage)
    ...
    func testInvalidOutputErrorWithJSONScenarios() {
        ...
        for message in jsonErrorMessages {
            let error = CardanoCLIToolsError.invalidOutput(message)
    ...
    func testNodeNotSyncedError() {
        ...
        let error = CardanoCLIToolsError.nodeNotSynced(syncProgress)
    ...
    func testNodeNotSyncedErrorWithDifferentProgress() {
        ...
        for progress in progressValues {
            let error = CardanoCLIToolsError.nodeNotSynced(progress)
    ...
    func testUnsupportedVersionError() {
        ...
        let error = CardanoCLIToolsError.unsupportedVersion(currentVersion, minimumVersion)
    ...
    func testUnsupportedVersionErrorWithSemanticVersioning() {
        ...
        for scenario in versionScenarios {
            let error = CardanoCLIToolsError.unsupportedVersion(scenario.current, scenario.minimum)
    ...
    func testInvalidMultiSigConfigError() {
        ...
        let error = CardanoCLIToolsError.invalidMultiSigConfig(configMessage)
    ...
    func testInvalidMultiSigConfigErrorWithVariousIssues() {
        ...
        for issue in configIssues {
            let error = CardanoCLIToolsError.invalidMultiSigConfig(issue)
    ...
    func testFileNotFoundError() {
        ...
        let error = CardanoCLIToolsError.fileNotFound(filePath)
    ...
    func testFileNotFoundErrorWithDifferentPaths() {
        ...
        for path in filePaths {
            let error = CardanoCLIToolsError.fileNotFound(path)
    ...
    func testVersionMismatchError() {
        ...
        let error = CardanoCLIToolsError.versionMismatch(versionMessage)
    ...
    func testVersionMismatchErrorWithVariousScenarios() {
        ...
        for message in mismatchMessages {
            let error = CardanoCLIToolsError.versionMismatch(message)
    ...
    @Test("CardanoCLIToolsError conforms to Error protocol")
    func testErrorProtocolConformance() {
        let error = CardanoCLIToolsError.binaryNotFound("/test/path")
        
        // Test that it can be thrown and caught
        do {
            throw error
        } catch {
            #expect(error is CardanoCLIToolsError)
            
            if let cliError = error as? CardanoCLIToolsError {
                switch cliError {
    ...
    @Test("CardanoCLIToolsError conforms to LocalizedError protocol")
    func testLocalizedErrorProtocolConformance() {
        let testCases: [CardanoCLIToolsError] = [
    ...
    @Test("CardanoCLIToolsError equality comparison")
    func testErrorEqualityComparison() {
        // Test same error cases
        let error1 = CardanoCLIToolsError.binaryNotFound("/test/path")
        let error2 = CardanoCLIToolsError.binaryNotFound("/test/path")
        ...
        let error3 = CardanoCLIToolsError.binaryNotFound("/different/path")
        #expect(error1.errorDescription != error3.errorDescription)
        
        let error4 = CardanoCLIToolsError.processAlreadyRunning
        #expect(error1.errorDescription != error4.errorDescription)
    }
    
    // MARK: - Error Pattern Matching Tests
    
    @Test("CardanoCLIToolsError pattern matching")
    func testErrorPatternMatching() {
        let errors: [CardanoCLIToolsError] = [
            .binaryNotFound("/test/binary"),
    ...
    @Test("CardanoCLIToolsError preserves context information")
    func testErrorContextPreservation() {
        ...
        let binaryError = CardanoCLIToolsError.binaryNotFound(binaryPath)
        ...
        let commandError = CardanoCLIToolsError.commandFailed(command, errorMsg)
        ...
        let syncError = CardanoCLIToolsError.nodeNotSynced(syncProgress)
    ...
    @Test("CardanoCLIToolsError message formatting consistency")
    func testErrorMessageFormattingConsistency() {
        // Test that all error messages follow consistent formatting patterns
        let testCases: [(error: CardanoCLIToolsError, expectedPrefix: String)] = [
    ...
    @Test("CardanoCLIToolsError integration with throwing functions")
    func testErrorIntegrationWithThrowingFunctions() async throws {
        // Test that CardanoCLIToolsError can be thrown and caught properly
        func mockBinaryCheck(_ path: String) async throws {
            if path.isEmpty {
                throw CardanoCLIToolsError.binaryNotFound(path)
            }
        }
        
        func mockCommandExecution(_ command: [String]) throws -> String {
            if command.isEmpty {
                throw CardanoCLIToolsError.commandFailed(command, "Empty command not allowed")
            }
            return "success"
        }
        ...
        func mockVersionCheck(_ current: String, _ minimum: String) async throws {
            if current < minimum {
                throw CardanoCLIToolsError.unsupportedVersion(current, minimum)
            }
        }
        ...
        do {
            ...
            Issue.record("Expected CardanoCLIToolsError, got: \(type(of: error))")
        }
    }
}
