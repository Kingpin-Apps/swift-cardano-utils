...
struct BinaryExecutableTests {
    ...
    @Test("checkBinary handles special file paths")
    func testCheckBinaryWithSpecialPaths() async throws {
        // Test with paths that have spaces and special characters
        let tempDir = FileManager.default.temporaryDirectory
        let specialPath = tempDir.appendingPathComponent("test binary with spaces-\(UUID().uuidString)")
        
        defer {
            try? FileManager.default.removeItem(at: specialPath)
        }
        
        // Create an executable file with spaces in name
        let script = "#!/bin/bash\necho 'test'"
        try script.write(to: specialPath, atomically: true, encoding: .utf8)
        
        // Make it executable
        var attributes = try FileManager.default.attributesOfItem(atPath: specialPath.path)
        if let permissions = attributes[.posixPermissions] as? NSNumber {
            let newPermissions = NSNumber(value: permissions.uint16Value | 0o111) // Add execute bits
            attributes[.posixPermissions] = newPermissions
            try FileManager.default.setAttributes(attributes, ofItemAtPath: specialPath.path)
        }
        
        // Should not throw
        try MockBinaryExecutable.checkBinary(binary: FilePath(specialPath.path))
    }
